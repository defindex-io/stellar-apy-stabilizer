#!/usr/bin/env bash
#
# Sweep orphaned tokens out of the deployed BoostTreasury contract.
#
# "Orphan" = balance(token) - tracked campaign balances for that token:
# direct transfers, vault refunds, dust, mistaken sends. The contract refuses
# to send more than the orphan amount, so campaign-attributed funds stay safe
# (unlike transfer(), which draws down a specific campaign's budget).
#
...
#
# Usage:
#   Interactive:  ./rescue_orphan.sh
#   Positional:   ./rescue_orphan.sh <network> <source_account> <token> <amount> <to>
#
# <token>  is the token/SAC contract address to sweep.
# <amount> is the token's raw stroops/units (positive i128).
# The signer MUST be the BoostTreasury admin.
# The call fails if amount > orphan balance (InsufficientOrphanBalance, #4032).
...
# Override for exceptional congestion with:  INCLUSION_FEE=<stroops> ./rescue_orphan.sh
...
TOKEN=$(resolve_required  "Token/SAC contract id to sweep"     "${3-__UNSET__}")
...
echo "──────────────────────────────────────"
echo " (note: the call will fail if amount > orphan balance, #4032)"
echo "──────────────────────────────────────"

read -rp "Rescue orphan now? [y/N] " confirm
...
echo
echo "✅ Rescued $AMOUNT of orphaned $TOKEN to $TO"
