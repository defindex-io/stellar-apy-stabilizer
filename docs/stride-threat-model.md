...
flowchart TB
    ...
    subgraph TB_TREAS_STORE["boost-treasury storage"]
        ...
        ts_camp[(persistent: Campaign per vault + CampaignList)]:::store
    end
...
5. **treasury admin ops** — admin calls `register_campaign(vault, asset)`, `update_campaign`, `unregister_campaign`, `transfer`, `rescue_orphan`, `set_manager`, `propose_admin`, `accept_admin`.
...
| **D**enial of Service | **DoS.1** — Single hot bot key is the only authorized `fee_manager` and treasury `Manager`. Bot-signer outage stops automated fee adjustment / boost until manual `set_fee_manager` rotation. **DoS.2** — No global pause flag; cross-fleet incident response requires N partner-side `unregister_vault` calls. |
| **E**levation of Privilege | **Elevation.1** — (Step 5) Admin can call `transfer(vault, amount, to)` to send up to `campaign.available()` tokens to any address — by design (refunds, reallocation), documented power of the trusted admin role. **Elevation.2** — (Step 1) An attacker holding a partner's current vault Manager key could attempt to register the vault with `config.admin = attacker`, inheriting all proxy-side admin actions (`set_target_apy`, `set_fee_bounds`, passthroughs, `release_fees`) for that vault. |
...
| Threat | Mitigations |
|---|---|
| **S**poofing | **Spoof.1** / **Elevation.2** — **S1R1 (FIXED in code).** `register_vault` now asserts `admin == config.admin` and panics with `AdminMismatch` (#3024) if they differ. Tracked as audit finding H1. **S1R2.** Partner onboarding docs surface `config.admin` clearly before signing. |
| **T**ampering | **Tamper.1** — **T1R1.** Bot signer key managed through isolated secret-storage with restricted access; key rotation and access reviews on a defined cadence. **T1R2.** Off-chain event-stream monitoring on `FeesLocked`, `FeesDistributed`, `Boosted`; admin can rotate the bot key with one tx (`set_fee_manager(burner)` / `set_manager(burner)`) on suspicion. **T1R3.** Risk-accepted by design (audit M9): single-key compromise cannot redirect funds — `vault_fee_receiver` is partner-controlled; recovery via `unregister_vault` + `release_fees` or boost-treasury top-up. **Tamper.2** — **T2R1 (FIXED in code).** Admin-only `rescue_orphan(token, to, amount)` added to boost-treasury, bounded by `balance(token) - sum_of_campaign_available_for_this_token`. The `CampaignList` index enumerates campaigns by asset so the contract can compute the bound. The function cannot drain campaign-tracked funds by construction. Tracked as audit finding M3. |
| **R**epudiation | **Repudiate.1** — **R1R1.** Every state-changing call emits a typed event (`FeesLocked`, `FeesDistributed`, `Boosted`, `Deposited`, `Transferred`, `OrphanRescued`, role-rotation events). Off-chain monitoring archives event streams for cross-referencing with bot operational logs. Per-human attribution within DeFindex ops is out of scope of the contract layer. |
...
- **Did STRIDE uncover new design issues?** Yes. **Spoof.1 / Elevation.2** (`config.admin` not authenticated in `register_vault`) → fixed by H1's `AdminMismatch` assertion. **Tamper.2** (stranded orphan tokens) → fixed by M3's `rescue_orphan` + `CampaignList`. **DoS.2** (no global pause) → reviewed and risk-accepted because existing primitives already cover the use case.
- **Did the treatments adequately address the issues?** Yes for the code-level fixes (H1, M3, L2, L3, L4, L5 — all merged, all tests passing, both contracts build clean). Risk-accepted items (M5, M6, M7, M8, M9) have documented operational mitigations and explicit rationale.
